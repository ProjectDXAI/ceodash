...
export default function ActivityFeed() {
  ...
  const filtered = intents
    ...
    .sort(
      (a, b) =>
        new Date(a.submitted_at || a.planned_at).getTime() -
        new Date(b.submitted_at || b.planned_at).getTime(),
    );
  ...
  return (
    <div
      data-panel="activity"
      className="panel-ethereal panel-ethereal-delay-1 rounded-[20px] border border-[#2a2a2a] overflow-hidden flex flex-col"
    >
      {/* Header bar */}
      <div className="flex items-center justify-between p-4 bg-[#111] relative z-10 border-b border-[#1a1a1a]">
        <div className="flex items-center gap-2">
          <span className="text-base font-bold text-white">Activity</span>
          <span className="text-xs text-neutral-500">{filtered.length}</span>
        </div>
        <div className="flex items-center gap-2">
          <span
            className="text-[8px] font-mono text-[#3a2a2a] eva-ticker-dot"
      ...
      <div
        ref={scrollRef}
        className="max-h-[220px] overflow-y-auto p-0 "
        // style={{
        //   maskImage:
      ...
      >
        {filtered.map((intent) => {
          ...
          const ts =
            intent.mined_at || intent.submitted_at || intent.planned_at;
          const time = format(new Date(ts), "h:mm");

          return (
            <div
              key={intent.id}
              className="group flex items-start gap-0 py-1.5 rounded hover:bg-[#ffffff06] transition-colors"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {intent.rationale && (
                  <span className="text-xs text-neutral-500 ml-1.5">
                    —{" "}
                    {intent.rationale.length > 50
                      ? intent.rationale.slice(0, 50) + "…"
                      : intent.rationale}
                  </span>
                )}
              </div>
      ...
      <div className="px-4 py-1.5 bg-[#111] border-t border-[#1a1a1a] flex items-center gap-2 relative z-10">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <span
              className="led-3 w-1.5 h-1.5 rounded-full bg-[#2ecc71]"
              style={{ boxShadow: "0 0 4px #2ecc71" }}
            />
            <span
              className="led-1 w-1.5 h-1.5 rounded-full bg-[#e74c3c]"
              style={{ boxShadow: "0 0 4px #e74c3c" }}
            />
            <span
