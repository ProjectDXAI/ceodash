...
export default function Home() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (sbJson.success && sbJson.data.length > 0 && vaultsJson.success) {
        ...
        const vaultsData = vaultsJson.data as Record<
          string,
          {
            ceo_slug: string;
    ...
    async (currentSnapshot: PnlSnapshot) => {
      try {
        ...
        if (!json.success || !json.data?.length) {
          setHistoricalData((prev) => {
            const updated =
              prev.length > 0 ? [...prev, currentSnapshot] : [currentSnapshot];
            saveHistoricalData(updated);
            return updated;
          });
          return;
  ...
  return (
    <main className="min-h-screen relative">
      {/* Splash Screen */}
      {!splashDone && (
        <div className="fixed inset-0 z-100 flex flex-col items-center justify-center splash-checkerboard">
          <img
            src="/ceobnch-01.png"
            alt="CEO Bench"
      ...
      {/* Header */}
      <header className="relative z-40 px-6 py-2 flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <a
            ...
            className="liquid-glass w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center hover:scale-110 active:scale-95"
            title="X / Twitter"
          >
            <svg
          ...
          <div className="mii-card p-6 mb-6 border-[#3a2020]! bg-[#1f1515]!">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-[#888]">Tap refresh to try again!</p>
              </div>
            </div>
          </div>
        )}
